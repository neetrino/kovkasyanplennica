...
const FOOTER_LANG: LanguageCode = 'ru';

type FooterProps = {
  /** Cream footer styling on `/menu`; defaults to dark theme when unknown server-side. */
  isMenuPage?: boolean;
};

export function Footer({ isMenuPage = false }: FooterProps) {
  const translate = (path: string) => t(FOOTER_LANG, path);
      ...
      <section className="py-16 md:py-20 lg:py-24 relative z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className={`${isMenuPage ? 'text-[#2f3f3d]' : 'text-white'} text-4xl md:text-5xl lg:text-6xl font-light italic mb-4`}>
            {translate('home.footer.booking.title')}
           </h2>
          ...
          <p className={`${isMenuPage ? 'text-[#2f3f3d]/80' : 'text-white/80'} text-lg md:text-xl mb-8 font-light`}>
            {translate('home.footer.booking.description')}
          </p>
          <Link
            href="/desktops"
          ...
          >
            {translate('home.footer.booking.selectTableButton')}
          </Link>
        </div>
      </section>
      )}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8 md:gap-12 mb-12">
            <div className="-mt-10 space-y-0 md:-mt-14">
              <div className="flex items-center gap-3">
                <Link
                  href="/"
                  aria-label={translate('home.header.logoAlt')}
              ...
              >
                <span className="block">{translate('home.footer.descriptionLine1')}</span>
                <span className="block">{translate('home.footer.descriptionLine2')}</span>
                <span className="block">{translate('home.footer.descriptionLine3')}</span>
              </p>
              </div>
            </div>
            ...
            <div>
              <h3 className={`${isMenuPage ? 'text-[#2f3f3d]' : 'text-white'} text-sm font-bold uppercase tracking-[0.7px] mb-4`}>
                {translate('home.footer.navigation.title')}
              </h3>
            ...
            <div>
              ...
              <div className="mt-5">
                <a
                  ...
                  aria-label={translate('home.footer.socialMedia.instagram')}
                >
            ...
            <div>
              <h3 className={`${isMenuPage ? 'text-[#2f3f3d]' : 'text-white'} text-sm font-bold uppercase tracking-[0.7px] mb-6`}>
                {translate('home.footer.workingHours.title')}
              </h3>
              <div className="space-y-4 mb-6">
                <div className="flex items-center justify-between gap-4 text-sm">
                  <span className={isMenuPage ? 'text-[#2f3f3d]/75' : 'text-[#ececec]'}>{translate('home.footer.workingHours.weekdays')}</span>
                  <span className={isMenuPage ? 'text-[#2f3f3d]' : 'text-white'}>{translate('home.footer.workingHours.weekdaysTime')}</span>
                </div>
                <div className="flex items-center justify-between gap-4 text-sm">
                  <span className={isMenuPage ? 'text-[#2f3f3d]/75' : 'text-[#ececec]'}>{translate('home.footer.workingHours.weekend')}</span>
                  <span className={isMenuPage ? 'text-[#2f3f3d]' : 'text-white'}>{translate('home.footer.workingHours.weekendTime')}</span>
                </div>
              </div>
              <p className="text-[#FADAAC] text-xs mt-4">
                {translate('home.footer.workingHours.deliveryInfo')}
              </p>
            </div>
          </div>
          ...
          <div className={`border-t ${isMenuPage ? 'border-[#2f3f3d]/10' : 'border-white/10'} pt-6 mt-8`}>
            <div className="flex flex-col md:flex-row flex-wrap justify-center md:justify-start items-center gap-3 md:gap-6 text-sm">
              ...
              >
                {translate('home.footer.bottomLinks.deliveryPolicy')}
              </Link>
              <Link
                href="/terms"
                className={`${isMenuPage ? 'text-[#2f3f3d]/60 hover:text-[#2f3f3d]/80' : 'text-white/60 hover:text-white/80'} text-[11px] md:text-xs transition-colors`}
              >
                {translate('home.footer.bottomLinks.terms')}
              </Link>
              <Link
                href="/privacy"
                className={`${isMenuPage ? 'text-[#2f3f3d]/60 hover:text-[#2f3f3d]/80' : 'text-white/60 hover:text-white/80'} text-[11px] md:text-xs transition-colors`}
              >
                {translate('home.footer.bottomLinks.privacy')}
              </Link>
            </div>
          </div>
        </div>
