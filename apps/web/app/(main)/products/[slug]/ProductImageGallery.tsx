    ...
    <>
      <div className="flex gap-6 items-start">
            ...
            {visibleThumbnails.map((image, index) => {
              ...
              return (
                <button 
                  ...
                  className={`relative block w-full aspect-[3/4] rounded-3xl overflow-hidden border bg-white transition-all duration-300 flex-shrink-0 ${
                    isActive 
                ...
                >
                  <img 
                    src={image} 
                    alt="" 
                    className="absolute inset-0 size-full object-cover rounded-3xl transition-transform duration-300" 
                  />
                </button>
              );
            })}
        ...
        <div className="min-w-0 flex-1">
          <div className="relative aspect-square w-full bg-white rounded-3xl overflow-hidden group shadow-[0_2px_8px_rgba(0,0,0,0.06)]">
          {images.length > 0 ? (
            <img 
              src={images[currentImageIndex]} 
              alt={product.title} 
              className="absolute inset-0 size-full object-cover rounded-3xl transition-transform duration-500 group-hover:scale-105" 
            />
          ) : (
            <div className="absolute inset-0 flex items-center justify-center rounded-3xl text-gray-400">{t(language, 'common.messages.noImage')}</div>
          )}
