...
import type { UpdateSpinWheelPrizeInput } from "@/lib/services/spin-wheel/spin-wheel.types";
...
export async function PUT(req: NextRequest, context: RouteContext) {
  try {
    ...
    const body = (await req.json()) as UpdateSpinWheelPrizeInput;
    const result = await spinWheelService.updatePrize(id, body);
...
export async function DELETE(req: NextRequest, context: RouteContext) {
  try {
    ...
    const { id } = await context.params;
    const result = await spinWheelService.deletePrize(id);
    return NextResponse.json(result);
  } catch (error: unknown) {
    const apiError = toApiError(error, req.url);
    return NextResponse.json(apiError, { status: apiError.status });
