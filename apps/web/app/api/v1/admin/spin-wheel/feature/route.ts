...
export async function GET(req: NextRequest) {
  try {
    ...
    const result = await spinWheelService.getSpinWheelFeatureSetting();
...
export async function PATCH(req: NextRequest) {
  try {
      ...
      return NextResponse.json(
        {
          ...
          status: 400,
          detail: "Request body must include boolean `enabled`",
          instance: req.url,
        },
        { status: 400 }
      );
    ...
    const result = await spinWheelService.setSpinWheelFeatureEnabled(body.enabled);
    return NextResponse.json(result);
  } catch (error: unknown) {
    const apiError = toApiError(error, req.url);
    return NextResponse.json(apiError, { status: apiError.status });
