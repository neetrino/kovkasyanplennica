...
async function ensureProductAttributesTable() {
    ...
    try {
        ...
        if (error?.code === 'P2021' || error?.message?.includes('does not exist') || error?.message?.includes('product_attributes')) {
...
async function ensureProductReviewsTable() {
    ...
    try {
        ...
        // If table doesn't exist, create it
        if (error?.code === 'P2021' || error?.message?.includes('does not exist') || error?.message?.includes('product_reviews')) {
...
async function ensureProductVariantAttributesColumn() {
    ...
    try {
        ...
        if (error?.code === 'P2022' || error?.message?.includes('does not exist') || error?.message?.includes('product_variants.attributes') || error?.message?.includes('column') && error?.message?.includes('attributes')) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__ef293607._.js.map