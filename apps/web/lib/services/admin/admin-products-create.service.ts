...
class AdminProductsCreateService {
  ...
  /**
   * Duplicate product: creates a copy in draft status with unique title/slug.
   * Variant SKUs are regenerated by createProduct.
   */
  async duplicateProduct(sourceId: string) {
    ...
    const variants = (source.variants || []).map((v: {
      ...
      published?: boolean;
      options?: Array<{ valueId?: string | null; attributeKey?: string | null; value?: string | null }>;
    }) => ({
      price: v.price,
    ...
    const payload = {
      ...
      locale: "en",
      media: source.media || [],
      labels: (source.labels || []).map((l: { type: string; value: string; position: string; color?: string | null }) => ({
        type: l.type,
        value: l.value,
        position: l.position,
