...
/** Shop listing fetch cap — enough for row pagination while keeping RSC payload smaller. */
const PRODUCTS_SHOP_LIST_LIMIT = 72;

const PRODUCTS_PAGE_LANG: LanguageCode = 'ru';
...
async function getProducts(
  ...
  try {
    ...
    const filters = buildProductFilters(page, limit, PRODUCTS_PAGE_LANG, params);
...
async function ProductsPageSidebarSlot() {
  const { data: categoryTreeRoots } = await getCategoryTreeCached(PRODUCTS_PAGE_LANG);
...
async function ProductsPageMainSlot({
  ...
  const language = PRODUCTS_PAGE_LANG;
...
async function ProductsPageDecorativeBackground({
  ...
  showFullDecorativeBackground: boolean;
}) {
...
async function ProductsPageDecorativeBackgroundFromParams({
  searchParams,
}: {
  searchParams?: Promise<Record<string, string | string[] | undefined>>;
}) {
  const raw = searchParams ? await searchParams : {};
  return (
    <ProductsPageDecorativeBackground
      showFullDecorativeBackground={!firstParam(raw.category)}
    />
  );
}
...
type ProductsPageShellProps = {
  searchParams?: Promise<Record<string, string | string[] | undefined>>;
  /** When true, skip searchParams on decorative background (cacheable default route). */
  defaultDecorativeBackground?: boolean;
};

export function ProductsPageShell({
  searchParams,
  defaultDecorativeBackground = false,
}: ProductsPageShellProps) {
  return (
    <div className="w-full max-w-full bg-[#2F3F3D] relative">
      <Suspense fallback={null}>
        {defaultDecorativeBackground ? (
          <ProductsPageDecorativeBackground showFullDecorativeBackground />
        ) : (
          <ProductsPageDecorativeBackgroundFromParams searchParams={searchParams} />
        )}
      </Suspense>
